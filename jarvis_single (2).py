...
class SpeechToText:
    ...
    def __init__(self, engine: str = "faster-whisper", model_size: str = "base", language: str = "en", logger: JarvisLogger = None):
        ...
        self.sr = None  # Store the module reference
        ...
        # Verify AudioData is available
        if not hasattr(self.sr, 'AudioData'):
            self.logger.error("speech_recognition.AudioData not available")
            self.sr = None
            return
    ...
    def transcribe_from_mic(self, timeout: float = 10.0, phrase_time_limit: float = 30.0) -> Optional[str]:
        if self.sr is None or self.recognizer is None:
    ...
    def _transcribe_whisper(self, audio_data) -> Optional[str]:
        try:
            import numpy as np
    ...
    def adjust_for_ambient_noise(self, duration: float = 1.0):
        if self.sr is None or self.recognizer is None:
            self.logger.warning("Speech recognition not available for noise adjustment")
            return
        try:
            with self.sr.Microphone() as source:
...
class Jarvis:
    ...
    def shutdown(self):
        """Shutdown gracefully without raising SystemExit."""
...
Examples:
  ...
  python jarvis_single.py --check            # Check environment setup
    ...
    try:
        ...
        # Exit cleanly after run_interactive completes (e.g., after "exit" command)
        sys.exit(0)
    except SystemExit as e:
        # Re-raise SystemExit with the original code
